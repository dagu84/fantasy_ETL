...
def qb_transform(data, week):
    ...
    data = data.drop(columns=['Rank', 'G', 'FPTS/G', 'ROST', 'PCT', 'Y/A'])
    data = data.drop(index=[0,1]).reset_index(drop=True)
    column = ['id', 'passing_cmp', 'passing_att', 'passing_yards', 'passing_td', 'passing_int',
    ...
    data[[ 'fantasy_points']] = data[[ 'fantasy_points']].astype(float)
...
def pass_catcher_transform(wr, te, week):
    ...
    wr = wr.drop(columns=['Rank', 'G', 'FPTS/G', 'ROST', 'LG', '20+', 'Y/R'])
    wr = wr.drop(index=[0,1]).reset_index(drop=True)

    te = te.drop(columns=['Rank', 'G', 'FPTS/G', 'ROST', 'LG', '20+', 'Y/R'])
    te = te.drop(index=[0,1]).reset_index(drop=True)
    data = pd.concat([wr, te], ignore_index=True)

    column = ['id', 'receiving_rec', 'receiving_targets', 'receiving_yards', 'receiving_td',
...
def rb_transform(data, week):
    ...
    data = data.drop(columns=['Rank', 'G', 'FPTS/G', 'ROST', 'LG', '20+', 'Y/R', 'Y/A'])
    data = data.drop(index=[0,1]).reset_index(drop=True)

    column = ['id', 'rushing_att', 'rushing_yards', 'rushing_td',
              'receiving_rec', 'receiving_targets', 'receiving_yards', 'receiving_td', 'fumbles_lost', 'fantasy_points']
    data.columns = column
